...
export class GetGroupsService {
  ...
  // Método para obtener grupos con un user_id específico con autenticación
  getGroupsByUserId(): Observable<Group[]> {
    ...
    return this.authService.getToken().pipe(
      switchMap(token => {
        ...
        tap(() => this.loadingService.hide()) // Ocultar el indicador de carga al recibir la respuesta
      }),
      catchError(error => {
