...
export class DashboardAdminService {
  ...
  getNoSqlDbYears(): Observable<YearsResponse[]> {
    return this.httpClient.get<YearsResponse[]>(`${this.rootURL}api/v1/dashboard/country/get_last_years/`);
  }
  getNoSqlDbCounts(year: number): Observable<DashboardCounts> {
    let params = new HttpParams().set('year', year.toString());
    return this.httpClient.get<DashboardCounts>(`${this.rootURL}api/v1/dashboard/country/get_acumulated/`, {params});
  }

  populateNoSqlDb(): Observable<Status> {
    return this.httpClient.post<Status>(`${this.rootURL}api/v1/dashboard/populate`,{});
  }
}
